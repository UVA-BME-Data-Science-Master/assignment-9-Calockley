---
title: "BatchEffectsHwTemplate.rmd"
...
#this p value leads to the conclusion that it is statistically significant
...
major[which.min(H)]
min(H)
#F major is the hardest because it has the lowest acceptance rate
```
...
D)The sample size for the individual majors was notlarge enough to draw the correct
conclusion.
```{r}
...
C) It cannot be done with one line.
...
2. Say Y is M cross N,in the SVD Y = UDV transpose which of the following is not correct? 
A) DV transpose are the new coordinates for the projection U transpose Y
B) UD are the new coordinates for the projection YV
C) D are the coordinates of the projection U transpose Y
D) U transpose Y is a projection from an N-dimensional to M-dimensional subspace.
```{r}
#C.) D are the coordinates of the projection U transpose Y
```
...
```{r}
s <- svd(y)
t <- s$v[,1:2] # these are the first two PCs.
rownames(t)<-colnames(y)
round(t,2)
fit = s$u[,1:2]%*% (s$d[1:2]*t(t))
var(as.vector(fit))/var(as.vector(y))
#fit
```

5. Plot each of the estimated factors ordered by date. Use color to denote month. The first factor
